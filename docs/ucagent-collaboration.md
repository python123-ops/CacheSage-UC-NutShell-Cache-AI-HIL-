...
| RV-007 | 把上游 smoke 与 Python 23/23 一并当作 RTL 覆盖证据 | 自研事务没有驱动真实 DUT，无法形成端到端功能覆盖率 | 建立 36 点真实 DUT 覆盖模型，命中必须来自响应、内存侧、victim 或 probe 事件 | 同期记录达到 `34/36`，两个背压缺口保留 |
...
| RV-009 | 直接复用上游 `block_write` | 位置参数错位使 data 被传入 size；同源参考模型未报警 | 项目适配器显式传递 `address,size,cmd,mask,data`，另建独立 byte-mask Scoreboard | 同期记录为 199 次比较、0 失败 |
| RV-010 | 对当前 Cache 实例执行通用 flush | RTL 断言表明 DCache 禁止该激励 | 删除非法 flush，改为观察公开 `io_empty` 空闲恢复 | 同期记录为三个 seed、421 条事务，无 RTL fatal |

上表保留 RV-006～010 发生时的结果，不回写成后来指标。2026-07-13 的新一轮工程更新另行加入 `BackpressureTrace`：先预热可预测 cache line，再以真实 `io_in_*` 握手证明输入/响应背压，最终形成 437 条事务、36/36 覆盖和 207 次零失败比较。该更新不伪装成原有 review journal 的同期发现。

## 指标变化

| 项目 | 修正前 | 修正后 |
| --- | ---: | ---: |
| 真实 DUT 功能覆盖率 | 未导出 | `36/36（100.00%）` |
| 真实 DUT Scoreboard | 未建立独立比较 | `207` 次比较，`0` 失败 |
| RTL 代码覆盖率 | 仅有候选文件 | `898/1454（61.00%）` |
| 回归规模 | 上游单一 smoke | 定向场景 + seed `11/29/73` + 背压握手，共 `437` 条事务 |
| 一致性观测 | 无 | probe miss、clean hit、dirty hit、8 beat 数据返回、probe 后重访 |

## 证据边界

- `36/36` 是 Toffee 驱动 Picker 生成 DUT 后得到的功能覆盖率，不是 Python harness 的 `23/23`。
- `61.00%` 是 Verilator code coverage，对应 `coverage.dat` 中的行、条件、分支和翻转等代码活动总计，不等同于功能覆盖率。
- `rtl_input_backpressure` 和 `rtl_response_backpressure` 来自 `io_in_*` valid/ready 的真实等待；输入等待 2 周期、响应等待 3 周期，payload 稳定且响应按序，不通过人工标记补齐。
