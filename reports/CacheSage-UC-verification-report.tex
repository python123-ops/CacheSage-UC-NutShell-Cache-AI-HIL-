...
GitHub 仓库： & \href{https://github.com/python123-ops/CacheSage-UC-NutShell-Cache-AI-HIL-}{python123-ops/CacheSage-UC-NutShell-Cache-AI-HIL-} \\
报告身份： & python123 \\
提交基线： & 仓库当前默认分支 HEAD \\
报告日期： & 2026-07-13 \\
\end{tabular}
...
真实 RTL 回归采用定向场景、seed 11、29、73 和握手背压场景，共执行 437 条 DUT 事务；36 个功能覆盖点全部命中，Scoreboard 完成 207 次读数据比较且无失败。输入等待 2 周期、响应等待 3 周期，payload 稳定且响应按序。Verilator RTL code coverage 898/1454 (61.00\%)。所有 fault artifact 均为 injected fault 检出证据，不代表真实 NutShell RTL 已被确认存在缺陷。
...
RTL 功能覆盖率 & \textbf{36/36}，\textbf{100.00\%}；437 条真实 DUT 事务 \\
RTL Scoreboard & 207 次比较，0 个失败 \\
RTL 背压握手 & 输入等待 2 周期；响应等待 3 周期；payload 稳定且响应按序 \\
RTL 代码覆盖率 & Verilator RTL code coverage 898/1454 (61.00\%)；与功能覆盖率分栏记录 \\
\bottomrule
\end{tabularx}
...
本次 Linux smoke 运行在 Ubuntu 24.04 WSL2 环境中，系统依赖包含 make、CMake、GCC/G++、Verilator、SWIG 与 Python venv。Picker 安装后 \texttt{picker\ --check} 显示 C++ 与 Python 支持可用；Python venv 中固定 \code{pytoffee==0.3.0} 与 \code{toffee-test==0.3.0}，避免 PyPI 最新包之间的接口漂移。

上游 Example-NutShellCache 的 \texttt{make\ gen\_dut} 返回 0，完成 Picker 生成 Python DUT 与 Verilator build；\texttt{make\ test} 返回 0，pytest 记录为 \texttt{test/test\_smoke.py::test\_smoke\ PASSED}。该证据说明基础环境构建已经从准备态变为可执行 smoke。脚本同时扫描波形、generated DUT 与 coverage candidate 文件，只提交 manifest，不提交大型波形二进制。

\tightsection{限制与补充条件}
当前 Linux 环境已完成真实 DUT 三 seed 回归和握手背压场景。输入等待 2 周期、响应等待 3 周期，payload 稳定且响应按序；大型 FST 与 coverage.dat 留在本地忽略目录，仓库保存可复核摘要。

回归入口为 \texttt{python\ scripts/run\_rtl\_regression.py}。脚本驱动 Picker-generated DUT，保存逐覆盖点事件来源、Scoreboard 失败明细、FST 路径和 Verilator coverage 摘要。
...
测试 & \texttt{python\ -m\ unittest\ discover\ -s\ tests\ -v} \\
编译检查 & \texttt{python\ -m\ compileall\ -q\ src\ tests\ scripts} \\
计划导出 & \texttt{python\ -m\ cachesage\_uc.cli\ plan} \\
覆盖率样例 & \texttt{python\ -m\ cachesage\_uc.cli\ run\ --seed\ 11\ --count\ 96\ --output\ reports/sample-run-seed11.json} \\
真实 RTL 回归 & \texttt{python\ scripts/run\_rtl\_regression.py} \\
报告构建 & \texttt{python\ scripts/build\_verification\_pdf.py} \\
...
\tightsection{执行证据摘要}
\begin{enumerate}
\item 基础测试：\texttt{python\ -m\ unittest\ discover\ -s\ tests\ -v}，覆盖 CLI、证据模型、公开材料约束、上游适配和 cache verification core。
\item 编译检查：\texttt{python\ -m\ compileall\ -q\ src\ tests\ scripts}，用于确认源码、测试和脚本均可被 Python 编译器解析。
\item 覆盖率样例：\code{reports/sample-run-seed11.json} 记录 seed、transaction count、covered points 与 event counts。
\item smoke 记录：\code{reports/nutshell-smoke.json} 记录上游目录、\texttt{make\ gen\_dut}、\texttt{make\ test}、RTL artifact manifest 与 RTL code coverage 状态。
...
当前 Linux 回归已驱动真实 Picker DUT 完成 437 条事务，RTL 功能覆盖率为 36/36（100.00\%），Scoreboard 207 次比较且无失败；Verilator RTL code coverage 898/1454 (61.00\%)。
...
\item \texttt{reports/rtl-functional-coverage.json}：真实 DUT 36/36 功能覆盖率、背压握手、逐点来源和 Scoreboard 明细。
