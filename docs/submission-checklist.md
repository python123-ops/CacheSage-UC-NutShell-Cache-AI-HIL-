...
| RTL 功能覆盖 | `integration/nutshell/`、`reports/rtl-functional-coverage.json` | 437 条真实 DUT 事务，`36/36（100.00%）`，207 次 Scoreboard 比较、0 失败 |
| 背压握手 | `BackpressureTrace`、evidence v2 | 输入等待 2 周期、响应等待 3 周期，payload 稳定且 8 条响应按序 |
...
| waveform 或 transaction trace | 本轮本地 FST 为 223382 bytes；仓库提交 manifest 和摘要，不提交大型二进制 |
