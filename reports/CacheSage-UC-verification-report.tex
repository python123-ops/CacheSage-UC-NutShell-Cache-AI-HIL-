...
\tightsection{摘要}
CacheSage-UC 面向 UCAgent NutShell Cache 赛题，围绕 cache 验证中的数据一致性、事件顺序、replacement、byte mask、stall 与 reset 等高风险路径，构建了一套可复现的 Python 验证原型。仓库包含 Generator/CRV、Reference Model、Scoreboard、Coverage Collector、Fault Injection 与 Review Journal。当前 Python harness 在 seed 11、96 个 transaction 上达到 \textbf{23/23}，即 \textbf{100.00\%} 功能覆盖率，并对 5 类 injected fault 给出确定性检出证据。
...
本机 smoke 状态 & \texttt{missing\_dependencies} \\
