...
def render_markdown(data: ReportData, report_date: str) -> str:
    ...
    smoke_summary = (
        "Linux 环境已完成上游 `make gen_dut` 与 `make test` smoke；RTL/Toffee measured coverage 尚未由上游测试导出。"
        if smoke_complete
...
def render_tex(data: ReportData, report_date: str) -> str:
    ...
    smoke_summary = (
        "Linux 环境已完成上游 \\code{make gen\\_dut} 与 \\code{make test} smoke；RTL/Toffee measured coverage 尚未由上游测试导出。"
        if smoke_complete
    ...
    smoke_evidence_item = (
        "smoke 记录：\\code{reports/nutshell-smoke.json} 记录上游目录、\\code{make gen\\_dut}、\\code{make test} 与 pytest smoke 均已通过。"
        if smoke_complete
