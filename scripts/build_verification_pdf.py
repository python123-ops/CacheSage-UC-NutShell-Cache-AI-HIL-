...
def render_markdown(data: ReportData, report_date: str) -> str:
    ...
    dependency_note = data.smoke.get("dependency_note", "未记录")
...
CacheSage-UC 面向 UCAgent NutShell Cache 赛题，构建了一套可复现的 cache 验证原型。当前证据包括 Python harness、Generator/CRV、Scoreboard、Coverage、5 类 injected fault 以及人工复核记录。Python harness 在 seed 11、{data.transaction_count} 个 transaction 上达到 `{data.coverage_covered}/{data.coverage_total}`，即 `{data.coverage_percent:.2f}%` 覆盖率。报告同时记录 Picker/Toffee/NutShell 集成边界：RTL/Toffee 覆盖率未实测，不声称已发现真实 NutShell RTL bug。
...
- 事件计数：{event_summary}。
- RTL/Toffee 覆盖率未实测；当前 smoke 状态为 `{data.smoke.get('status')}`，缺失依赖：{missing_deps}。
...
\tightsection{{摘要}}
CacheSage-UC 面向 UCAgent NutShell Cache 赛题，围绕 cache 验证中的数据一致性、事件顺序、replacement、byte mask、stall 与 reset 等高风险路径，构建了一套可复现的 Python 验证原型。仓库包含 Generator/CRV、Reference Model、Scoreboard、Coverage Collector、Fault Injection 与 Review Journal。当前 Python harness 在 seed 11、{data.transaction_count} 个 transaction 上达到 \textbf{{{data.coverage_covered}/{data.coverage_total}}}，即 \textbf{{{data.coverage_percent:.2f}\%}} 功能覆盖率，并对 5 类 injected fault 给出确定性检出证据。

本报告的核心目标是给评审提供可审计材料：哪些能力已经由脚本和测试证明，哪些环节仍属于 Picker/Toffee/NutShell 的本机集成边界。当前 RTL/Toffee 覆盖率未实测，相关字段保留为环境记录；所有 fault artifact 均为 injected fault 检出证据，不代表真实 NutShell RTL 已被确认存在缺陷。
...
该边界是有意保守的：Picker/Toffee 真实运行需要本机具备 make、picker、toffee 和 toffee-test。当前报告只记录上游 layout inspection、Toffee-style request preview 与 Python harness 结果，避免把环境准备状态写成 RTL 实测结论。

\tightsection{{限制与补充条件}}
本报告的主要限制是 RTL/Toffee 覆盖率未实测。该限制来自本机缺失 Picker、Toffee、Toffee-Test 或 make，不来自 Python harness 自身无法运行。为保持证据可信度，报告将这部分写成集成环境记录，并保留 \code{{rtl_toffee_measured_coverage: null}} 的机器可读字段。
...
\item smoke 记录：\code{{reports/nutshell-smoke.json}} 记录上游目录已就绪，同时明确本机缺失 make、picker、toffee 与 toffee-test。
...
后续若在 Linux 或完整 EDA 环境中安装 Picker、Toffee、Toffee-Test 与 make，可将同一场景矩阵接入 Picker-generated DUT，并把 RTL/Toffee measured coverage、waveform 片段和真实 RTL smoke 结果追加到报告中。当前版本坚持证据边界，不把尚未实测的 RTL 结果写成已完成结论。
