...
class NutShellAdapterTests(unittest.TestCase):
    def test_example_layout_reports_required_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "Makefile").write_text("gen_dut:\n\t@echo ok\n", encoding="utf-8")
...
class AiHilEvidenceTests(unittest.TestCase):
    def test_ai_hil_jsonl_has_required_human_review_fields(self):
        rows = [
            json.loads(line)
            for line in (ROOT / "ai_hil_log.jsonl").read_text(encoding="utf-8").splitlines()
            if line.strip()
        ]
        ...
        for row in rows:
            ...
            self.assertTrue(row["ai_output_summary"])
            self.assertTrue(row["human_finding"])
